...
# Grouping data on basis of continents ?üê?
contData = df.groupby("continent")
# Average happinessScore per continent ?ü¶?
...
def similar():
 ...
 'Spain', 'Uruguay' ,'Malta'],
  ...
  return(dict)
#Îß§Í∞úÎ≥??àòÎ°? :countryÍ∞? ?ì§?ñ¥?ò§Î©? Í∑? countryÍ∞? ?Üç?ïú Í∑∏Î£π?ùò ?Çò?ùº?ì§?ùÑ Î™®Îëê Ï∂úÎ†•?ï¥Ï£ºÎäî Î°úÏßÅ

##???Î•ôÎ≥ÑÎ°? TOP10 ?Ç¥Î≥¥ÎÇ¥Í∏?##
...
def result(country):
  ...
  for i in ['dystopia','gdp','socialSupport','health','freedom','generosity','corruptionPerceptions']:
    ...
    globals()['{}_text'.format(i)]= '?Üí' if globals()['{}_per'.format(i)]<=50 else '?ÇÆ'
